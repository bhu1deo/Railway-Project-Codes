...
df_list = []
df_list_min = []
for s in station_list:            # For every station we will have a dataframe 
    d = {}
    d_list = []
    for train in TDDc:
    ...
    df_list.append(df)                                # df contains in HHMM 
...
for d in tqdm(df_list_up):
    d = d.reset_index()
    del d['index']
    d['Arr_dif'] = d['Arrival'].diff()                 # Successive arrivals difference 
...
for i in list(df.index):
    if(i%2==0):
        df.iloc[i,5] = '-'            # This difference row is the 5th row 
    
df['Arrival'] = df['Arrival'].apply(lambda x: min_to_HHMM(int(float(x)))[0])                  # Because HHMM values can't be compared so only after processing convert to HHMM 
df['Departure'] = df['Departure'].apply(lambda x: min_to_HHMM(int(float(x)))[0])        
