...
for i in overtake_df.index.tolist():
    if(overtake_df.iloc[i]['Overtaken_T.No.'][0]=='2'):
        if(overtake_df.iloc[i]['Overtaken_T.No.'] not in overtake_stations_dict):
            ...
            overtake_stations_dict[overtake_df.iloc[i]['Overtaken_T.No.']].append(overtake_df.iloc[i]['stn_code'])      # Again only stations are required here 
        else:
            ...
            for lis in traversal_dict_list[train]:                 #  REQ traversal_dict_list
                ...
                if(s in lis):
                    ...
                    if(pure_halt==True and lis!=traversal_dict_list[train][-1]):
                        arr = float(HHMM_to_min(lis[2]))-float(scheduled_dict[train][s])       # arrival is changed dep remains same 
                        arr_lis.append(min_to_HHMM_mod(arr))
                    elif(pure_halt==True and lis==traversal_dict_list[train][-1]):             # Last station arrival = departure
                        arr_lis.append(lis[2])
                    elif(no_halt==True):
                        arr_lis.append(lis[2])
                    else:
                        arr_lis.append(lis[1])                     # Keep the actual arrival time 
