...
for i in tqdm(df.index.tolist()):
    ...
    allowance_dict[df.iloc[i]['Train_no']] = dic
...
print("\nDone getting the acceleration AND deceleration losses for trains....")
...
unsch = open(infraPath + 'unscheduled1.txt').readlines()[2:]             # Read in Satwik's unscheduled.txt
...
for u in tqdm(unsch):
    ...
    if((loop_dict[end_loop] in exceptionStationList) and (u.split()[1]=='up')):             # Only for up direction trains, for down direction trains the problem is automatically taken care of as the IC point is Lonavla instead of Kalyan 
...
#             print(loop_dict[start_loop],loop_dict[end_loop])
            ...
            ea_rate = allowance_dict[train_number[2:]]['ea_rate']
            ta_rate = allowance_dict[train_number[2:]]['ta_rate']
            ...
            if(total_allowance<7):           # Only one halt
                if(total_allowance-acc_dec_loss[speed_dict[train_number]]*0.8>0): # Allowance can indeed be allocated
                    if(direction_dict[train_number]=='up'):         # NDLS to MMCT
                        prev_station = stn_list_up[stn_list_up.index(loop_dict[end_loop])-1]   # previous station before the destination
